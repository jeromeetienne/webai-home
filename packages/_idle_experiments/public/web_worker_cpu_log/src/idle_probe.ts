...
/**
 * Identical in method to `visibility_timer_log/src/idle_probe.ts`, kept as its own copy so this
 * experiment's main-thread baseline is measured exactly the same way, independently of the
 * worker thread `cpu_worker.ts` measures at the same time.
 */

/** What kind of measurement one log row reports. */
export type LogRowKind = 'visibility' | 'focus' | 'timer' | 'raf' | 'cpu' | 'worker-cpu' | 'worker-timer';
