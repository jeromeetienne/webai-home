...
/**
 * A dedicated worker thread has no `document` and no visibility state of its own. Whether its
 * own timers and computation slow down when the page that owns it is hidden is exactly what
 * this file measures, by running the identical fixed workload `visibility_timer_log` runs on
 * the main thread, here instead — so the two are directly comparable in `main.ts`'s combined log.
 */

/** Loop iterations per benchmark run, identical to `visibility_timer_log`'s so the two are directly comparable. */
