...
type Model = {
	...
	fullName: string;
	id: string;
	/** Pinned Hugging Face revision, so a moving `main` branch cannot change what the gate measured. */
	revision: string;
	dtype: 'q4' | 'q4f16';
	accent: 'amber' | 'blue';
	kind: 'text' | 'gemma4';
...
const models: Record<string, Model> = {
	...
	// The issue #96 de-risk gate: a real ONNX Runtime Web + WebGPU run of the pinned
	// onnx-community/Qwen3.5-0.8B-ONNX export, proving the merged decoder's hybrid
	// linear-attention/full-attention cache (past_conv/past_recurrent plus
	// past_key_values on every fourth layer) actually loads and generates.
	qwen3_5: {
		shortName: 'Qwen3.5',
		fullName: 'Qwen3.5-0.8B-ONNX (gate)',
