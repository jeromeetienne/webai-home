...
socket.addEventListener("message", (event) => {
  ...
  if (message.type === "registered") status.textContent = "Connected to the central server.";
...
socket.addEventListener("close", () => { status.textContent = "Disconnected from the central server."; });
