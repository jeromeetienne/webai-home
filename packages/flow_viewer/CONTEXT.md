...
- `src/cli.ts`: the `flow_viewer` command line program. It scans `packages/gateway/logs` for files named `gateway-*.log_entry.jsonl`, serves the page, and opens a browser. Run it with `npm run flow_viewer --workspace @webai/flow-viewer`.
...
- `web/src/playback_controller.ts`, `web/src/event_log_panel.ts`, `web/src/statistics.ts`, `web/src/types.ts`, and `web/src/theme_toggle.ts`.
