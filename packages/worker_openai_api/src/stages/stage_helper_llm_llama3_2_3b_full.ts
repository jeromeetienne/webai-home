...
export class StageHelperLlmLlama3_2_3bFull {
	...
	static async compute(
		...
		try {
			const reader = state.reader ?? await StageHelperLlmLlama3_2_3bFull.startGeneration(state, payload.text ?? '', openaiApiClient, modelId);
	 ...
	 * @throws If the prompt is empty, if the request fails, or if the assignment was taken away
	 * while the request was starting.
	 */
	private static async startGeneration(
		...
		const stream = await openaiApiClient.chatCompletionStream(modelId, prompt, abortController);
