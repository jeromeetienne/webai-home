...
- `web/src/page/`: `page_markup.ts`, `page_elements.ts`, `worker_event_log.ts`, `theme_toggle.ts`, and `audio_keepalive.ts`, which plays a quiet tone so a backgrounded tab is not throttled.
