...
- `stage_helper_llm_qwen3_5_0_8b_full.ts` reports an exact `promptTokenCount` (from `generator.tokenizer.apply_chat_template`), an exact `completionTokenCount` (from `TextStreamer`'s `token_callback_function`), and a `stopReason` told apart by `criteria.interrupted` and the model's own `eos_token_id` set, all confirmed live by milestone 0's de-risk gate. `stage_helper_llm_gemma_nano_chrome_full.ts` reports only `stopReason`, always `end_of_sequence` — this engine has no prompt/completion token count to report at all, only a cumulative context-window usage number in its own unit, and an interrupted answer never reaches the line that reports usage since a session destroyed mid-read throws instead. See milestone 3 of [issue #150](https://github.com/webai-at-home/webai-at-home/issues/150).
