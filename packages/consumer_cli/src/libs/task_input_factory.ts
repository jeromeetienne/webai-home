 ...
 * complete Qwen3.5-0.8B model, whose worker applies the model's chat template through
 * `@huggingface/transformers`, and the complete Llama 3.2 3B model, whose worker forwards the
 * messages to a local server that speaks the OpenAI-compatible API.
 *
 ...
 */
export const taskTypeNamesAcceptingConversation = ['llm_qwen3_5_0_8b_full', 'llm_llama3_2_3b_full'] as const;
...
export class TaskInputFactory {
	...
	static createTaskInput(
		...
		return { taskType: 'task_type_llm_llama3_2_3b_full', input: TaskInputFactory.parseLlmOrConversationInput(value), ...settings };
	}
