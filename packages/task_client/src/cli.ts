...
import { createTaskInput, TaskClient } from "./task_client.js";
...
export class MainHelper {
	...
	static async run(args: string[] = process.argv.slice(2)): Promise<void> {
		...
		const socket = new WebSocket(options.url) as unknown as ConstructorParameters<typeof TaskClient>[0];
		const client = new TaskClient(socket, {
			onMessage: (direction, message) => messageLogger.log(direction, { role: "server" }, message.type, message),
