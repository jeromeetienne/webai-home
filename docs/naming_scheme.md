...
This part says how the model is arranged, and not which program holds it. A model held complete on one device is `full` whether the worker downloads and runs the model itself or forwards the prompt to a server already running on that device, because those are the same arrangement seen by whoever submits the task. `task_type_llm_llama3_2_3b_full` is therefore named `full` even though a local server speaking the OpenAI-compatible API is what actually runs it, and which server that is — Ollama, LM Studio, or another — is an option of the worker process rather than part of the name. `nano_chrome` names its runtime for a different reason: that model cannot be separated from the browser that ships it, so the browser is part of which model is being run rather than part of how it is arranged. This was the open naming question of [issue #100](https://github.com/webai-at-home/webai-at-home/issues/100), decided in [issue #103](https://github.com/webai-at-home/webai-at-home/issues/103).
...
| `task_llm_llama3_2_3b_full` | `task_type_llm_llama3_2_3b_full` | `llm_llama3_2_3b_full@1` | `stage_llm_llama3_2_3b_full` | `llm_llama3_2_3b_full` |

The three shard stages of `llm_qwen3_0_6b_sharded@1` run as a group once per generated token, because that pipeline sets `repeatsUntilDone`. The single stage of `llm_gemma_nano_chrome_full@1` runs once per piece of the answer the browser's own language model produces, for the same reason. The single stage of `llm_qwen3_5_0_8b_full@1` runs the same way, once per piece of the answer the downloaded model produces, and so does the single stage of `llm_llama3_2_3b_full@1`, once per piece of the answer the local server produces. The two stages of `dev_formula@1` each run once.
...
- The task type accepted by the consumer command line option `-t/--type` is the task type name without the leading `task_type_`, so `dev_formula`, `llm_qwen3_0_6b_sharded`, `llm_gemma_nano_chrome_full`, `llm_qwen3_5_0_8b_full`, and `llm_llama3_2_3b_full`. The five are listed once, in `taskTypeNames` in [`packages/consumer_cli/src/libs/task_input_factory.ts`](../packages/consumer_cli/src/libs/task_input_factory.ts), and [`packages/consumer_cli/src/cli.ts`](../packages/consumer_cli/src/cli.ts) builds both its help text and its check from that list.
- The model identifiers the OpenAI-compatible server offers are the same five names, for the same reason: a model identifier there is the task type name without the leading `task_type_`. `ModelCatalog` in [`packages/consumer_openai/src/api/model_catalog.ts`](../packages/consumer_openai/src/api/model_catalog.ts) builds both the list returned by `GET /v1/models` and the check on a request's `model` field from the same `taskTypeNames`, so the models on offer cannot drift away from the task types the cluster runs.
- The sample commands in [`packages/consumer_cli/package.json`](../packages/consumer_cli/package.json) are `sample:dev_formula`, `sample:llm_qwen3_0_6b_sharded`, `sample:llm_gemma_nano_chrome_full`, `sample:llm_qwen3_5_0_8b_full`, and `sample:llm_llama3_2_3b_full`.
- The consumer names those sample commands register under are the same words with underscores written as hyphens, because they are display names rather than identifiers: `dev-formula-consumer`, `llm-qwen3-0-6b-sharded-consumer`, `llm-gemma-nano-chrome-full-consumer`, `llm-qwen3-5-0-8b-full-consumer`, and `llm-llama3-2-3b-full-consumer`.
- A task whose worker is not a browser tab has no debug page, because those pages exist to open worker browser tabs inside inline frames. `task_llm_llama3_2_3b_full` is run by the Node.js worker in [`packages/worker_openai_api`](../packages/worker_openai_api), which is started from a command line instead, so no `/debug_iframe_llm_llama3_2_3b_full` page exists.
- The gateway debug pages are served at `/debug_iframe_dev_formula`, `/debug_iframe_llm_qwen3_0_6b_sharded`, `/debug_iframe_llm_gemma_nano_chrome_full`, and `/debug_iframe_llm_qwen3_5_0_8b_full`, and each inline frame in them is named after the single stage it is restricted to, with underscores written as hyphens: `dev-formula-multiply`, `dev-formula-add`, `llm-qwen3-0-6b-shard1of3`, `llm-qwen3-0-6b-shard2of3`, `llm-qwen3-0-6b-shard3of3`, `llm-gemma-nano-chrome-full`, and `llm-qwen3-5-0-8b-full`.
- The worker source file that implements the computations of one domain is named after those computations, as `stage_helper_{task_suffix}.ts`: [`packages/worker_webpage/web/src/stages/stage_helper_dev_formula.ts`](../packages/worker_webpage/web/src/stages/stage_helper_dev_formula.ts), [`packages/worker_webpage/web/src/stages/stage_helper_llm_qwen3_0_6b_sharded.ts`](../packages/worker_webpage/web/src/stages/stage_helper_llm_qwen3_0_6b_sharded.ts), [`packages/worker_webpage/web/src/stages/stage_helper_llm_gemma_nano_chrome_full.ts`](../packages/worker_webpage/web/src/stages/stage_helper_llm_gemma_nano_chrome_full.ts), and [`packages/worker_webpage/web/src/stages/stage_helper_llm_qwen3_5_0_8b_full.ts`](../packages/worker_webpage/web/src/stages/stage_helper_llm_qwen3_5_0_8b_full.ts). A worker that is not a browser names its own stage files the same way, in its own package: [`packages/worker_openai_api/src/stages/stage_helper_llm_llama3_2_3b_full.ts`](../packages/worker_openai_api/src/stages/stage_helper_llm_llama3_2_3b_full.ts).
