...
- The task type accepted by the consumer command line option `-t/--type` is the task type name without the leading `task_type_`, so `dev_formula`, `llm_qwen3_0_6b_sharded`, `llm_gemma_nano_chrome_full`, and `llm_qwen3_5_0_8b_full`. The four are listed once, in `taskTypeNames` in [`packages/consumer_cli/src/libs/task_input_factory.ts`](../packages/consumer_cli/src/libs/task_input_factory.ts), and [`packages/consumer_cli/src/cli.ts`](../packages/consumer_cli/src/cli.ts) builds both its help text and its check from that list.
- The model identifiers the OpenAI-compatible server offers are the same four names, for the same reason: a model identifier there is the task type name without the leading `task_type_`. `ModelCatalog` in [`packages/consumer_openai/src/libs/model_catalog.ts`](../packages/consumer_openai/src/libs/model_catalog.ts) builds both the list returned by `GET /v1/models` and the check on a request's `model` field from the same `taskTypeNames`, so the models on offer cannot drift away from the task types the cluster runs.
- The sample commands in [`packages/consumer_cli/package.json`](../packages/consumer_cli/package.json) are `sample:dev_formula`, `sample:llm_qwen3_0_6b_sharded`, `sample:llm_gemma_nano_chrome_full`, and `sample:llm_qwen3_5_0_8b_full`.
- The consumer names those sample commands register under are the same words with underscores written as hyphens, because they are display names rather than identifiers: `dev-formula-consumer`, `llm-qwen3-0-6b-sharded-consumer`, `llm-gemma-nano-chrome-full-consumer`, and `llm-qwen3-5-0-8b-full-consumer`.
