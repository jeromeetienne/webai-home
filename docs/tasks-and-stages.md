...
The stage sets `prefersSameWorkerOnRetry`, and this is what keeps the whole answer on one device. The session and the reader that delivers the answer live in the memory of the browser tab that started them, so no other device can carry on an answer that was begun somewhere else. Because the stage sets it, every run after the first is placed on the tab recorded against the stage in the task's `stageWorkerDeviceIds`, which is the tab holding the open answer, rather than on whichever tab advertising the stage happens to be free. A stage run that says it continues an answer, but reaches a browser tab holding no answer for that task, fails with a message saying so, rather than reading the answer so far as if it were a new prompt.
...
A second tab advertising the same stage does no harm. The gateway records which tab ran the stage for each task and places every later run of that answer back on the same tab, so two tabs advertising this stage run two answers side by side rather than each being handed pieces of an answer it cannot continue. A later run still reaches a different tab in the cases where the preference cannot be honoured: the tab holding the answer has disconnected, has stopped offering the stage, or is already running as many assignments as its own limit allows. That run fails with a message saying this browser is not holding the answer, and the task fails once its attempts are used up.
