...
- `v` is the protocol version the frame was written for. The current version is 4, declared by `protocolVersion` in [`packages/protocol/src/message/envelope_types.ts`](../packages/protocol/src/message/envelope_types.ts). No earlier version is accepted.
